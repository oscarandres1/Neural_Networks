...
# Hidden and output nuerons for MLP network
HIDDEN_NEURONS = 7
OUT_NEURONS = 1

def crear_modeloFF():
    inputs1 = Input(shape=(PASOS, 1), name='Input_1')
    dense1 = Dense(HIDDEN_NEURONS , name='MLP_1', activation = 'relu')(inputs1)
    drop1 = Dropout(0.1)(dense1)
    dense2 = Dense(HIDDEN_NEURONS , name='MLP_2', activation = 'relu')(drop1)
    flatten_1 = Flatten()(dense2)
    cnn1D = Conv1D(filters=4, kernel_size=7, strides=1, activation='relu', padding="same")(inputs1) #6,7
    mxPl1D = MaxPooling1D(pool_size=2)(cnn1D)
    flatten_2 = Flatten()(mxPl1D)
    drop3 = Dropout(0.1)(flatten_2)
    concatenated_1 = Concatenate( name='Concatenate_1')([drop3,flatten_1])
    ...
    plot_model(model, to_file='CNN_MLP', show_shapes=True)
    model.summary()
    return model
...
hist_df.to_csv('historyCNNMLP.csv')
#----------------------------------------
#plt.title('Accuracy')
...
realValues.to_csv('entrenamientoCNNMLP.csv')
...
realValues = pd.DataFrame(inverted)
realValues.columns = ['Real', 'Prediccion']
realValues.to_csv('testCNNMLP.csv')
realValues['Real'].plot(label = 'Real')
realValues['Prediccion'].plot(label = 'Predicted')
plt.legend()
plt.title('Test')
...
plt.show()
prediccionMeses.to_csv('pronosticoCNNMLP.csv')
