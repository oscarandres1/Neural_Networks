...
from keras.layers import Dense,Activation,Flatten, LSTM, Concatenate, Dropout, Flatten
...
def crear_modeloFF():
    model = Sequential()
    model.add(LSTM(HIDDEN_NEURONS, input_shape=(PASOS,1),return_sequences = True, activation = 'relu'))
    ...
    model.summary()
    plot_model(model, to_file='LSTM_sequetial', show_shapes=True)
    return model
...
hist_df.to_csv('historyLSTMSequential.csv')
#----------------------------------------
#plt.title('Accuracy')
...
realValues.to_csv('entrenamientoLSTMSequential.csv')
...
realValues = pd.DataFrame(inverted)
realValues.columns = ['Real', 'Prediccion']
realValues.to_csv('testLSTMSequential.csv')
realValues['Real'].plot(label = 'Real')
realValues['Prediccion'].plot(label = 'Predicted')
plt.legend()
plt.title('Test')
...
plt.show()
prediccionMeses.to_csv('pronosticoLSTMSequential.csv')
'''	
