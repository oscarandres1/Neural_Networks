...
from keras.layers import Dense,Activation,Flatten, LSTM, Input, Concatenate, Dropout, Flatten
...
def series_to_supervised(data, n_in=1, n_out=1, dropnan=True):  #n_in or out = 1 es por default, pero pasa 12
    n_vars = 1 if type(data) is list else data.shape[1]   #ver si el data frame es un vetor 1x n_muestras
    df = pd.DataFrame(data)      #lo pone como pandadataframe
    cols, names = list(), list()   #crea listas vacias
    # input sequence (t-n, ... t-1)
    for i in range(n_in, 0, -1):  #empieza en 12 = PASOS , termina en 1 (no incluye 0)
...
#df = pd.read_csv('datos1.csv',  parse_dates=[0], #header=None,index_col=0, squeeze=True,names=['fecha','unidades'])
#print(df.head())
# cargar datos como fechas y float
df['Meses'] = pd.to_datetime(df['Meses'])
df['Exportaciones'] = df['Exportaciones'].astype('float32')

#| -- Poner la columna “Meses” como el índice de nuestro dataset, así Python sabrá que este conjunto de datos es una serie de tiempo 
...
print(reframed.shape) #queda en 236 x13
...
#[:, -1] it means all the rows and only the last column. -1 represents the last column.
#[:, :-1], it means all the rows and all the columns except the last column.
...
HIDDEN_NEURONS = 7 # 9, 350, 12 # 11, 350, 7
OUT_NEURONS = 1

def crear_modeloFF():
    model = Sequential() 
    model.add(Dense(HIDDEN_NEURONS, input_shape=(PASOS,1), activation='relu'))
    ...
    plot_model(model, to_file='MLP_sequetial', show_shapes=True)
    model.summary()
    return model
...
hist_df.to_csv('historyMLPSequential.csv')
#----------------------------------------
#plt.title('Accuracy')
...
realValues.to_csv('entrenamientoMLPSequential.csv')
...
realValues = pd.DataFrame(inverted)
realValues.columns = ['Real', 'Prediccion']
realValues.to_csv('testnMLPSequential.csv')
realValues['Real'].plot(label = 'Real')
realValues['Prediccion'].plot(label = 'Predicted')
plt.legend()
plt.title('Test')
...
plt.show()
prediccionMeses.to_csv('pronosticoMLPSequential.csv')
'''
